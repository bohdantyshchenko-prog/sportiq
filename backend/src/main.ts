...
const env = z.object({
  ...
  PORT: z.coerce.number().int().positive().default(8080), APP_ORIGIN: z.string().url(), DATABASE_URL: z.string().min(1),
  SUPABASE_URL: z.string().url(), SUPABASE_JWT_ISSUER: z.string().url(), SUPABASE_JWT_AUDIENCE: z.string().default('authenticated'),
  FOOTBALL_DATA_BASE_URL: z.string().url().default('https://api.football-data.org/v4'), FOOTBALL_DATA_TOKEN: z.string().min(1),
  OPENAI_API_KEY: z.string().min(1), OPENAI_MODEL: z.string().default('gpt-5-mini'),
...
const app = Fastify({ logger: true, requestTimeout: 15_000, bodyLimit: 256_000, trustProxy: true, disableRequestLogging: env.NODE_ENV === 'production' });
...
const normalizeMatch = (match: any): Prisma.MatchUncheckedCreateInput => ({
  id: `fd-${String(match.id)}`, provider: 'football-data', providerMatchId: String(match.id), tournament: String(match.competition?.name ?? 'Unknown'),
  homeTeam: String(match.homeTeam?.name ?? 'Home'), awayTeam: String(match.awayTeam?.name ?? 'Away'), startsAt: new Date(match.utcDate),
  status: String(match.status ?? 'SCHEDULED').toLowerCase(), payload: match as Prisma.InputJsonValue
});
...
app.post('/v1/ai/briefing', { config: { rateLimit: { max: 12, timeWindow: '1 minute' } } }, async request => {
  ...
  const match = await prisma.match.findUnique({ where:{ id:input.matchId } }); if (!match) throw Object.assign(new Error('MATCH_NOT_FOUND'), { statusCode:404 });
  const response = await openai.responses.create({ model:env.OPENAI_MODEL, input:[{role:'system',content:'You are NOVIQ. Separate confirmed facts, derived signals, unknowns and confidence. Never invent injuries, lineups or statistics.'},{role:'user',content:JSON.stringify({locale:input.locale,match})}], text:{format:{type:'json_schema',name:'briefing',strict:true,schema:{type:'object',additionalProperties:false,properties:{facts:{type:'array',items:{type:'string'}},signals:{type:'array',items:{type:'string'}},unknowns:{type:'array',items:{type:'string'}},confidence:{type:'integer',minimum:0,maximum:100},summary:{type:'string'}},required:['facts','signals','unknowns','confidence','summary']}}} });
