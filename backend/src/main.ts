...
app.addHook('onRequest', async (request, reply) => {
  ...
  try {
    ...
    if (!verified.payload.sub) throw new Error('MISSING_SUB');
    request.user = { id: verified.payload.sub, email: typeof verified.payload.email === 'string' ? verified.payload.email : null };
  } catch {
...
declare module 'fastify' {
  interface FastifyRequest { user: { id: string; email: string | null }; }
}
...
const sportsRequest = async (path: string) => {
  ...
  if (!response.ok) {
    const providerError = new Error(`SPORTS_PROVIDER_${response.status}`);
    (providerError as Error & { statusCode?: number }).statusCode = response.status === 429 ? 503 : 502;
    throw providerError;
  }
...
app.get('/ready', async (_request, reply) => {
  try { await prisma.$queryRaw`SELECT 1`; return { status: 'ready', version: VERSION }; }
  catch { return reply.code(503).send({ status: 'not_ready' }); }
});

app.get('/v1/me', async request => prisma.profile.upsert({
...
app.get('/v1/matches', async request => {
  ...
  const query = z.object({ dateFrom: z.string().regex(date).optional(), dateTo: z.string().regex(date).optional() }).parse(request.query);
  const from = query.dateFrom ?? new Date().toISOString().slice(0, 10);
  const to = query.dateTo ?? new Date(Date.now() + 7 * 86400000).toISOString().slice(0, 10);
  const fromDate = new Date(`${from}T00:00:00Z`); const toDate = new Date(`${to}T00:00:00Z`);
  if (fromDate > toDate || toDate.getTime() - fromDate.getTime() > 31 * 86400000) {
    return { error: 'INVALID_DATE_RANGE', data: [] };
  }
...
app.post('/v1/ai/briefing', { config: { rateLimit: { max: 12, timeWindow: '1 minute' } } }, async request => {
  ...
  const response = await openai.responses.create({
    ...
    input: [
      { role: 'system', content: 'You are NOVIQ. Separate confirmed facts, derived signals, unknowns and confidence. Never invent injuries, lineups or statistics.' },
...
app.post('/v1/ai/ask', { config: { rateLimit: { max: 12, timeWindow: '1 minute' } } }, async request => {
  const input = z.object({ question: z.string().min(3).max(1000), context: z.record(z.unknown()).default({}) }).parse(request.body);
  const response = await openai.responses.create({
    model: env.OPENAI_MODEL,
    input: [
      { role: 'system', content: 'Answer as NOVIQ. Be concise, separate verified facts from inference, and explicitly state unknowns.' },
...
app.post('/v1/push/subscribe', async request => {
  const s = z.object({ endpoint: z.string().url(), keys: z.object({ p256dh: z.string().min(1), auth: z.string().min(1) }), userAgent: z.string().max(500).optional() }).parse(request.body);
  return prisma.pushDevice.upsert({
    where: { endpoint: s.endpoint },
    update: { profileId: request.user.id, p256dh: s.keys.p256dh, auth: s.keys.auth, userAgent: s.userAgent },
    create: { profileId: request.user.id, endpoint: s.endpoint, p256dh: s.keys.p256dh, auth: s.keys.auth, userAgent: s.userAgent }
  });
});
